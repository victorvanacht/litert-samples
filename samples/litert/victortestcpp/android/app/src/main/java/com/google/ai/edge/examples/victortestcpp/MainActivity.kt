...
import com.google.ai.edge.examples.victortestcpp.view.ApplicationTheme
...
private fun ModelRunnerScreen(
  ...
  onSelectGpuPrecision: (GpuPrecision) -> Unit,
  onSelectGpuBackend: (GpuBackend) -> Unit,
  onSelectGpuPriority: (GpuPriority) -> Unit,
  onSelectGpuBufferStorageType: (GpuBufferStorageType) -> Unit,
  ...
  Column(modifier = Modifier.fillMaxSize()) {
    TopAppBar(title = { Text("victortestcpp") }, backgroundColor = MaterialTheme.colors.secondary)
    Column(
...
private fun GpuPrecisionSelector(
  selected: GpuPrecision,
  onSelect: (GpuPrecision) -> Unit,
) {
  var expanded by remember { mutableStateOf(false) }
  Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
    Text("GPU precision")
...
private fun GpuBackendSelector(
  ...
  Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
    ...
    OutlinedButton(onClick = { expanded = true }) { Text(selected.name) }
    DropdownMenu(expanded = expanded, onDismissRequest = { expanded = false }) {
      GpuBackend.entries.forEach { backend ->
...
private fun GpuPrioritySelector(
  selected: GpuPriority,
  onSelect: (GpuPriority) -> Unit,
) {
  var expanded by remember { mutableStateOf(false) }
  Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
    Text("GPU priority")
    OutlinedButton(onClick = { expanded = true }) { Text(selected.name) }
    DropdownMenu(expanded = expanded, onDismissRequest = { expanded = false }) {
      GpuPriority.entries.forEach { priority ->
...
private fun GpuBufferStorageSelector(
  selected: GpuBufferStorageType,
  onSelect: (GpuBufferStorageType) -> Unit,
) {
  var expanded by remember { mutableStateOf(false) }
  Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
    ...
    OutlinedButton(onClick = { expanded = true }) { Text(selected.name) }
    DropdownMenu(expanded = expanded, onDismissRequest = { expanded = false }) {
      GpuBufferStorageType.entries.forEach { storageType ->
